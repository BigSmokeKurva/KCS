...
[ApiController]
[Route("api/auth")]
public class AuthApiController(DatabaseContext db) : ControllerBase
{
    [HttpPost]
    [Route("signup")]
    public async Task<ActionResult> Registration([FromBody] RegistrationModel model)
    {
        ...
        var inviteCodeValid =
            await db.InviteCodes.AnyAsync(x => x.Code == model.InviteCode && x.Status == InviteCodeStatus.Active);
        if (!(loginValid && passwordValid && inviteCodeValid))
        {
            ...
            if (!passwordValid)
                data.errors.Add(new ErrorResponseModel
                {
                    type = "password",
                    message =
                        "Пароль должен содержать только буквы латинского алфавита (заглавные и строчные), цифры и символы !@#$%^&*()_-."
                });
    ...
    public async Task<ActionResult> Authorization([FromBody] AuthorizationModel model)
    {
        ...
        var user = await db.Users.FirstOrDefaultAsync(
            x => x.Username == model.Login && x.Password == model.Password);
        if (user is null)
        {
            var data = new ResponseModel
            {
        ...
        var authToken = await db.Sessions.AddAsync(new Session
        {
            Id = user.Id,
            Expires = TimeHelper.GetUnspecifiedUtc().AddDays(30)
        });
        await db.AddLog(user, "Авторизовался.", LogType.Action);
        await db.SaveChangesAsync();
        ...
        Response.Cookies.Append("auth_token", authToken.Entity.AuthToken.ToString(), cookieOptions);
        // редирект на главную страницу
        return Ok(new
        {
            status = "ok"
    ...
    {
        var authToken = Guid.Parse(Request.Cookies["auth_token"]!);

        if (!await db.Sessions.AnyAsync(x => x.AuthToken == authToken))
            return Ok(new { status = "ok" });
        db.Sessions.Remove((await db.Sessions.FindAsync(authToken))!);
        await db.AddLog(
            await db.Users.FirstAsync(x => x.Sessions.Any(y => y.AuthToken == authToken && x.Id == y.Id)),
            "Вышел из аккаунта.", LogType.Action);
        await db.SaveChangesAsync();
