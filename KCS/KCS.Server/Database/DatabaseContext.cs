...
public class DatabaseContext(DbContextOptions<DatabaseContext> options) : DbContext(options)
{
    ...
    public DbSet<FilterWord> FilterWords { get; init; }
    public DbSet<BotInfo> Bots { get; init; }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<User>(entity =>
        {
            ...
            entity.Property(e => e.LastOnline).HasColumnName("last_online").HasColumnType("timestamp")
                .HasDefaultValue(TimeHelper.GetUnspecifiedUtc());
        ...
        modelBuilder.Entity<Configuration>(entity =>
        {
            ...
            entity.Property(e => e.StreamerInfo).HasColumnType("jsonb").HasColumnName("streamerInfo");
    ...
    internal async Task<int> GetId(Guid authToken)
    {
        return await Sessions.Where(x => x.AuthToken == authToken).Select(x => x.Id).FirstOrDefaultAsync();
    }

    internal async Task<User> GetUser(Guid authToken)
    {
        return await Users.FirstAsync(x => Sessions.First(y => y.AuthToken == authToken).Id == x.Id);
    }

    internal async Task<Configuration> GetConfiguration(Guid authToken)
    {
        return await Configurations.FirstAsync(x => Sessions.First(y => y.AuthToken == authToken).Id == x.Id);
    }
